...
struct WifiManager
{
        ...
        , provisioning_ssid_(provisioning_ssid)
    {
        ...
        ESP_ERROR_CHECK(esp_event_handler_register(PROTOCOMM_SECURITY_SESSION_EVENT, ESP_EVENT_ANY_ID, &WifiManager::event_handler, this));
        ...
        if (!provisioned_)
        {
            ...
            ESP_ERROR_CHECK(wifi_prov_mgr_start_provisioning(WIFI_PROV_SECURITY_0, NULL, provisioning_ssid.c_str(), NULL));
        }
        else
        {
            ...
            switch (event_id) {
                ...
                case WIFI_PROV_CRED_FAIL: {
                    ...
                    if (wifi_manager->retries_ >= WIFI_CONNECTION_MAX_RETRIES) {
                        ESP_LOGI(wifi_manager->TAG, "Failed to connect with provisioned AP, reseting provisioned credentials");
