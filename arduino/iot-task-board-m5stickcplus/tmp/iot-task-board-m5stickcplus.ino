// This is a program to measure manipulation performance with the Smart Task Board. 
...
const unsigned long trialPublishRate = 1 * 0.50 * 1000UL; //500ms //a rate of 500ms seems to be too fast and clogs the logic of the controller in practice.
...
void publish_telemetry(){
      DynamicJsonDocument telemetry(1023); // increased from 1023, then 8192
...
void develop_screen(){
    ...
    M5.Lcd.printf("Smart Task Board ");
    M5.Lcd.printf("v%s\n ", FW_VERSION);
    M5.Lcd.printf("WiFi On:%d Status:%d TrialCount: %s\n ", wifiEnabled, WiFi.status(), trialCounter);
    M5.Lcd.printf("Token: %s\n ", TOKEN.c_str());
    M5.Lcd.printf("PROTOCOL: %s\n ", PROTOCOL_ID);
...
void screen5(){
    ...
    M5.Lcd.printf("Probe in Red Port & Tip in White Port: %s\n ", probeStartState);
    M5.Lcd.printf("Probe in Red Port & Tip in Terminal Block: %s\n ", probeGoalState);    
...
void send_telemetry()
{
  ...
  {
    Serial.println("WiFi is disabled, no telemetry sent.");
  }
}

void check_trial_status()
{
//Start Trial on M5 Button Press Check
  if (startBtnState == BUTTON_ON && trialRunning == 0 && stopBtnState == BUTTON_OFF && forceStop == 0) 
  {
    ...
    if (probeGoalState == BUTTON_ON){
      // flash alert screen to remove probe plug from red port
      M5.Lcd.setCursor(5,5);
      M5.Lcd.setTextColor(WHITE, RED);
      M5.Lcd.setTextSize(1);
      M5.Lcd.fillScreen(RED);
    ...
    if (faderValue == 20){
      // flash alert screen to move slider to end (edge of board)
      M5.Lcd.setCursor(5,5);
      M5.Lcd.setTextColor(WHITE, RED);
      M5.Lcd.setTextSize(1);
      M5.Lcd.fillScreen(RED);
      M5.Lcd.printf("Move slider to end (edge of board)!");
      delay(1000);
      M5.Lcd.setTextColor(WHITE, BLACK);
      M5.Lcd.fillScreen(BLACK);
    }
...
void print_trial_status_to_serial()
{
...
void loop()
{
  ...
  send_telemetry();
  check_trial_status();
  print_trial_status_to_serial();
  screen_switcher_proc();
