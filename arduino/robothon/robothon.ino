...
void loop()
{
  ...
  if (wifiEnabled == 1)
  {
    ...
    if (now - lastPublish >= fiveSeconds) // publish to topic every 5 seconds
    {
      ...
      telemetry[0]["Fader_TS"] = TS_fader; //INT
      telemetry[0]["Angle_TS"] = TS_angle; //INT
      telemetry[0]["CableWrap_TS"] = TS_cableWrap; //INT
      telemetry[0]["ProbeGoal_TS"] = TS_probeGoal; //INT
...
//      telemetry[0]["PROTOCOL"] = PROTOCOL_ID; //STR 
  ...
  if (startBtnState != BUTTON_OFF && trialRunning == 0 && faderValue < 10 && angleValue < 10 && probeStartState == 0) 
  {
    delay(1);
  ...
  if (stopBtnState != BUTTON_OFF && trialRunning == 1 && buttonPushLatch == 1 && faderLatch == 1 && angleLatch == 1 && cableWrapLatch == 1 && probeGoalLatch == 1)
  {
    delay(1);
  ...
  //Angle Check
  if (angleValue > ANGLESP && trialRunning == 1 && angleLatch == 0)
  {
    delay(1);
  ...
  //Restore Probe Check
  if (probeGoalState != BUTTON_OFF && trialRunning == 1 && probeGoalLatch == 0)
  {
    probeGoalLatch = 1;
