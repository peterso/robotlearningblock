...
int keyswitchRState = -1;
int keyswitchLState = -1;
int plugState = -1;
int portRState = -1;
int portLState = -1;
int batt1BtnState = -1;
...
void loop()
{
  ...
  keyswitchLState = porthub.hub_d_read_value_A(HUB_ADDR[3]);
  keyswitchRState = porthub.hub_d_read_value_B(HUB_ADDR[3]);
  plugState = porthub.hub_d_read_value_A(HUB_ADDR[2]);
  portRState = porthub.hub_d_read_value_A(HUB_ADDR[2]);
  portLState = porthub.hub_d_read_value_B(HUB_ADDR[2]);
...
//  if (startBtnState != BUTTON_OFF && trialRunning == 0 && plugState == 1 && keyswitchState == 1) 
  if (startBtnState == BUTTON_ON && trialRunning == 0 && forceStop == 0) //removing plug position requirement
  {
    delay(1);
    if (portLState == 1){ // plugged in is 0
      // flash alert screen to move ethernet plug
      M5.Lcd.setCursor(5,5);
      M5.Lcd.setTextColor(WHITE, RED);
      M5.Lcd.setTextSize(1);
      M5.Lcd.fillScreen(RED);
      M5.Lcd.printf("Move ethernet plug to port near M5!");
      delay(1000);
      M5.Lcd.setTextColor(WHITE, BLACK);
      M5.Lcd.fillScreen(BLACK);
    }
    delay(1);
    if (keyswitchState == 0){
      // flash alert screen to move key to the center position
      M5.Lcd.setCursor(5,5);
      M5.Lcd.setTextColor(WHITE, RED);
      M5.Lcd.setTextSize(1);
      M5.Lcd.fillScreen(RED);
      M5.Lcd.printf("Move key switch to the center position!");
      delay(1000);
      M5.Lcd.setTextColor(WHITE, BLACK);
      M5.Lcd.fillScreen(BLACK);
    }
    delay(1);
    // if (startBtnState == BUTTON_ON && portLState == 0 && keyswitchState == 1 && usecCount == 0 && countStart == 0){
    if (startBtnState == BUTTON_ON && portLState == 0){
      // begin a trial
      countStart = 1;
      startaccX = accX;
      startaccY = accY;
...
//      timeLeft = 0;
  ...
  //Keyswitch Check
  if (keyswitchState != BUTTON_OFF && trialRunning == 1 && keyswitchLatch == 0)
  {
    delay(1);
    keyswitchLatch = 1;
    TS_key = usecCount;
  ...
  //Plug Check
  if (plugState != BUTTON_OFF && trialRunning == 1 && plugLatch == 0)
  {
    delay(1);
  ...
  if (batt1BtnState != BUTTON_OFF && trialRunning == 1 && batt1Latch == 0)
  {
    delay(1);
    batt1Latch = 1;
    TS_batt1 = usecCount;
  ...
  if (batt2BtnState != BUTTON_OFF && trialRunning == 1 && batt2Latch == 0)
  {
    batt2Latch = 1;
    TS_batt2 = usecCount;
